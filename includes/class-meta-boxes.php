 ...
 *  - Meta Tagi SEO            (now includes sitemap priority + changefreq)
 *  - Tagi OpenGraph
 *  - Schema JSON SEO
 *
 * Uses standard add_meta_box() API which is fully compatible with both
 * the Classic Editor and Gutenberg.
 *
...
final class Meta_Boxes {
	...
	public function render_seo_meta_box( \WP_Post $post ): void {
		...
		<div class="pb-seo-wrap">
			...
			<p class="pb-seo-row">
				<label for="pb_seo_pagePrioritySitemap"><strong><?php esc_html_e( 'Priorytet strony w sitemapie', 'pb-media-all-seo' ); ?></strong></label>
				<select id="pb_seo_pagePrioritySitemap" name="pb_seo[pagePrioritySitemap]" class="widefat">
					<option value=""><?php esc_html_e( '— Domyślnie (0.7) —', 'pb-media-all-seo' ); ?></option>
					<?php foreach ( Sitemap::VALID_PRIORITIES as $opt ) : ?>
						<option value="<?php echo esc_attr( $opt ); ?>" <?php selected( $values['pagePrioritySitemap'], $opt ); ?>>
							<?php echo esc_html( $opt ); ?>
						</option>
					<?php endforeach; ?>
				</select>
				<span class="description"><?php esc_html_e( 'Wartość tagu <priority> w sitemapa.xml dla tej strony.', 'pb-media-all-seo' ); ?></span>
			</p>

			<p class="pb-seo-row">
				<label for="pb_seo_pageChangeFreqSitemap"><strong><?php esc_html_e( 'Częstotliwość dla strony w sitemapie', 'pb-media-all-seo' ); ?></strong></label>
				<select id="pb_seo_pageChangeFreqSitemap" name="pb_seo[pageChangeFreqSitemap]" class="widefat">
					<option value=""><?php esc_html_e( '— Domyślnie (weekly) —', 'pb-media-all-seo' ); ?></option>
					<?php foreach ( Sitemap::VALID_CHANGEFREQ as $opt ) : ?>
						<option value="<?php echo esc_attr( $opt ); ?>" <?php selected( $values['pageChangeFreqSitemap'], $opt ); ?>>
							<?php echo esc_html( $opt ); ?>
						</option>
					<?php endforeach; ?>
				</select>
				<span class="description"><?php esc_html_e( 'Wartość tagu <changefreq> w sitemapa.xml dla tej strony.', 'pb-media-all-seo' ); ?></span>
			</p>
		</div>
		<?php
	}
	...
	public function save_meta_boxes( int $post_id, \WP_Post $post ): void {
		...
		// New sitemap fields.
		$this->save_enum_field( $post_id, 'pagePrioritySitemap', $raw, Sitemap::VALID_PRIORITIES );
		$this->save_enum_field( $post_id, 'pageChangeFreqSitemap', $raw, Sitemap::VALID_CHANGEFREQ );
	...
	private function save_html_meta_tags( int $post_id, string $key, array $raw ): void {
		...
		$allowed = [
			'meta' => [
				'name' => true, 'property' => true, 'content' => true,
				'http-equiv' => true, 'charset' => true, 'scheme' => true,
			],
			'link' => [
				'rel' => true, 'href' => true, 'type' => true,
				'sizes' => true, 'media' => true, 'title' => true, 'hreflang' => true,
			],
		];
